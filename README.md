...
| **Conformidade** | A **consciência**: não produz — **veta** e **marca**. Julga o disclosure de mídia sintética, sinaliza queda de autenticidade (anti-slop), confere o licenciamento dos ativos, veta temas inapropriados na borda da Descoberta e guarda uma trilha de auditoria por vídeo. Regras versionadas por tipo. **Objetivas bloqueiam, subjetivas avisam.** *(transversal)* | `conformidade/` |

Cada pilar tem um `README.md` próprio detalhando seus módulos. **Controle** mantém dois lares existentes e já bem nomeados (`api/` para a app web, `config/` para as camadas de configuração). **Conformidade** nasce **inerte** (`conformidade.ativo=false`) — com ela desligada, o comportamento é o de hoje. Pastas de dados de runtime (`output/`, `execucoes/`, `tendencias/`, e por tipo `tipos/<id>/feedback/`, `tipos/<id>/guia/` e `tipos/<id>/conformidade/`) e o conteúdo por tipo (`tipos/`) não são código e ficam na raiz por padrão. As raízes base (saída/render, execuções, tendências e o conteúdo por tipo) são **configuráveis pelo painel** — o bloco `caminhos` em `config/sistema.json`, resolvido por `config/caminhos.py` — para apontar o armazenamento a um mount (NAS) sem editar código; os defaults são exatamente os locais de hoje, então nada se move até você mudar o ajuste.
